...
/// Central de Ocorrências (F27) — Ciclo de Sincronização e Envio ao SPP.
///
/// 4 estados mockados mutáveis:
/// • Rascunho → Continuar
/// • A sincronizar → Sincronizar → Concluída
/// • Concluída → Enviar ao SPP → Enviada ao SPP
/// • Enviada ao SPP (somente leitura)
...
class _OcorrenciasScreenState extends State<OcorrenciasScreen> {
  ...
  // ═══════════════════════════════════════════════════════════════
  // Dados mock mutáveis (persistem durante a sessão)
  // ═══════════════════════════════════════════════════════════════
  ...
  List<Map<String, dynamic>> get _filtradas {
    return _ocorrencias.where((o) {
      ...
      switch (_filtroStatus) {
        case 'Rascunhos': if (s != 'Rascunho') return false;
        case 'Concluídas': if (s != 'Concluída') return false;
        case 'A sincronizar': if (s != 'A sincronizar') return false;
        case 'Enviadas ao SPP': if (s != 'Enviada ao SPP') return false;
      }
      if (_buscaCtrl.text.isNotEmpty) {
        final q = _buscaCtrl.text.toLowerCase();
        final searchable = '${o['protocolo']} ${o['vitima']} ${o['bairro']} ${o['municipio']} ${o['nic']}'.toLowerCase();
  ...
  Color _statusBarColor(String status) {
    switch (status) {
      case 'Rascunho': return PCPEColors.primary;
      case 'A sincronizar': return PCPEColors.warning;
      case 'Concluída': return PCPEColors.success;
      case 'Enviada ao SPP': return PCPEColors.primaryDark;
      default: return PCPEColors.primary;
    }
  }
  ...
  Widget _buildStatusChip(String status) {
    ...
    switch (status) {
      case 'Rascunho':
        color = PCPEColors.primary; bg = PCPEColors.primarySoft; icon = Icons.edit_note; break;
      case 'A sincronizar':
        color = PCPEColors.warning; bg = PCPEColors.warningLight; icon = Icons.sync_problem; break;
      case 'Concluída':
        color = PCPEColors.success; bg = PCPEColors.successLight; icon = Icons.check_circle; break;
      case 'Enviada ao SPP':
        color = PCPEColors.primaryDark; bg = PCPEColors.cardGray; icon = Icons.send; break;
      default:
        color = PCPEColors.primary; bg = PCPEColors.primarySoft; icon = Icons.circle;
    }
    return Container(
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: PCPEColors.primary, foregroundColor: PCPEColors.pureWhite, elevation: 0), onPressed: () { Navigator.pop(ctx); setState(() => _ocorrencias[index]['status'] = 'Concluída'); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Ocorrência sincronizada com sucesso.'), backgroundColor: PCPEColors.success, behavior: SnackBarBehavior.floating)); }, child: const Text('Sincronizar')),
        ],
      ),
    );
  }
  ...
  Widget _infoLine(String label, String value) => Padding(
    ...
    child: Row(children: [
      ...
      Expanded(child: Text(value, style: const TextStyle(fontSize: 12, color: PCPEColors.black))),
    ]),
  );

  void _onCardTap(Map<String, dynamic> o) {
    final s = o['status'] as String;
    if (s == 'Rascunho') { context.go('/nova-ocorrencia'); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Redirecionando para ${o['protocolo']}...'), backgroundColor: PCPEColors.darkGray, behavior: SnackBarBehavior.floating)); }
    else { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Visualização em desenvolvimento.'), backgroundColor: PCPEColors.darkGray, behavior: SnackBarBehavior.floating)); }
  }
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => Padding(
        ...
        child: Wrap(spacing: 12, runSpacing: 12, children: [
          const Text('Filtros Avançados', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
          const SizedBox(width: 999),
          PCPEInput(hint: 'Município', prefixIcon: Icons.location_city),
          PCPEInput(hint: 'Bairro', prefixIcon: Icons.map),
          PCPEInput(hint: 'Natureza', prefixIcon: Icons.gavel),
          PCPEInput(hint: 'Unidade', prefixIcon: Icons.business),
          const Text('Filtros mockados — implementação completa em sprint futura.', style: TextStyle(fontSize: 11, color: PCPEColors.mediumGray, fontStyle: FontStyle.italic)),
        ]),
      ),
    );
  }

  Widget _miniIndicator(String label, int value, Color color) => Container(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: PCPEColors.pureWhite, elevation: 0, toolbarHeight: 56,
        title: const Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
          Text('Central de Ocorrências', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: PCPEColors.black)),
          Text('Gerencie os registros de locais de crime.', style: TextStyle(fontSize: 11, color: PCPEColors.mediumGray)),
        ]),
        actions: [
          if (!isMobile) Padding(padding: const EdgeInsets.only(right: 8), child: PCPEButton(label: 'Nova Ocorrência', icon: Icons.add, small: true, height: 38, onPressed: () => context.go('/nova-ocorrencia'))),
          if (isMobile) IconButton(icon: const Icon(Icons.add, color: PCPEColors.primary), tooltip: 'Nova Ocorrência', onPressed: () => context.go('/nova-ocorrencia')),
        ],
      ),
      body: Column(children: [
        // ── Resumo Operacional (5 indicadores) ─────────────────
        Container(
          width: double.infinity,
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
          decoration: BoxDecoration(color: PCPEColors.pureWhite, border: Border(bottom: BorderSide(color: PCPEColors.surfaceGray))),
          child: Wrap(spacing: 8, runSpacing: 8, alignment: WrapAlignment.spaceAround, children: [
            _miniIndicator('Registradas', _ocorrencias.length, PCPEColors.primary),
            _miniIndicator('Rascunhos', _countByStatus('Rascunho'), PCPEColors.primary),
            _miniIndicator('Concluídas', _countByStatus('Concluída'), PCPEColors.success),
            _miniIndicator('A sincronizar', _countByStatus('A sincronizar'), PCPEColors.warning),
            _miniIndicator('Enviadas ao SPP', _countByStatus('Enviada ao SPP'), PCPEColors.primaryDark),
          ]),
        ),
        // ── Busca + Filtros ────────────────────────────────────
        Padding(
          ...
          child: Row(children: [
            Expanded(child: PCPEInput(hint: 'Pesquisar por protocolo, NIC, vítima ou endereço...', prefixIcon: Icons.search, controller: _buscaCtrl, onChanged: (_) => setState(() {}))),
            const SizedBox(width: 8),
            PCPEButton(label: 'Filtros', icon: Icons.tune, outlined: true, small: true, height: 44, onPressed: _showFiltrosDialog),
          ]),
        ),
        SizedBox(
          height: 42,
          child: ListView.separated(
            ...
            itemCount: _filtros.length, separatorBuilder: (_, __) => const SizedBox(width: 6),
            itemBuilder: (c, i) {
              final f = _filtros[i]; final sel = f == _filtroStatus;
              return FilterChip(
                label: Text(f, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: sel ? PCPEColors.pureWhite : PCPEColors.darkGray)),
                selected: sel, showCheckmark: false, selectedColor: PCPEColors.primary, backgroundColor: PCPEColors.pureWhite,
                side: BorderSide(color: sel ? PCPEColors.primary : PCPEColors.surfaceGray),
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(18)), padding: const EdgeInsets.symmetric(horizontal: 10),
                visualDensity: VisualDensity.compact, onSelected: (_) => setState(() => _filtroStatus = f),
              );
            },
          ),
        ),
        ...
        Padding(
          ...
          child: Row(children: [
            const Icon(Icons.folder_outlined, size: 16, color: PCPEColors.mediumGray), const SizedBox(width: 6),
            Text('${ocorrencias.length} registros encontrados', style: const TextStyle(fontSize: 12, color: PCPEColors.mediumGray)),
          ]),
        ),
        const SizedBox(height: 6),
        Expanded(
          child: ocorrencias.isEmpty
              ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.search_off, size: 48, color: PCPEColors.mediumGray.withValues(alpha: 0.4)), const SizedBox(height: 12), const Text('Nenhuma ocorrência encontrada', style: TextStyle(fontSize: 14, color: PCPEColors.mediumGray))]))
              : ListView.builder(
                  padding: const EdgeInsets.only(bottom: 16), itemCount: ocorrencias.length,
                  itemBuilder: (context, index) {
                    ...
                    final realIndex = _ocorrencias.indexOf(o);

                    return Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 3),
                      child: Material(
                        color: PCPEColors.pureWhite, borderRadius: BorderRadius.circular(10), elevation: 0.5,
                        child: InkWell(
                          borderRadius: BorderRadius.circular(10), onTap: () => _onCardTap(o),
                          child: Container(
                            decoration: BoxDecoration(borderRadius: BorderRadius.circular(10), border: Border(left: BorderSide(color: barColor, width: 4))),
                            padding: const EdgeInsets.fromLTRB(12, 10, 12, 10),
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              Row(children: [
                                Expanded(child: Text(o['protocolo'] as String, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: PCPEColors.black))),
                              ...
                              Row(children: [
                                ...
                                const Icon(Icons.photo_camera_outlined, size: 12, color: PCPEColors.mediumGray), const SizedBox(width: 3),
                                Text('${o['fotos']} fotos', style: const TextStyle(fontSize: 11, color: PCPEColors.mediumGray)),
                              ]),
                              if (ehEnviada && o['dataEnvioSpp'] != null) ...[
                                const SizedBox(height: 2),
                              ...
                              const SizedBox(height: 2),
                              Row(children: [
                                const Icon(Icons.access_time, size: 12, color: PCPEColors.mediumGray), const SizedBox(width: 3),
                                Text('Atualizada há ${o['alteracao']}', style: const TextStyle(fontSize: 10, color: PCPEColors.lightGray)),
                                const Spacer(),
                                if (ehRascunho) ...[
                                  PCPEButton(label: 'Continuar', icon: Icons.edit, small: true, height: 30, onPressed: () { context.go('/nova-ocorrencia'); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Redirecionando para edição...'), backgroundColor: PCPEColors.darkGray, behavior: SnackBarBehavior.floating)); }),
                                  const SizedBox(width: 6),
                                  PCPEButton(label: 'Visualizar', icon: Icons.visibility_outlined, small: true, height: 30, outlined: true, onPressed: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Visualização em desenvolvimento.'), backgroundColor: PCPEColors.darkGray, behavior: SnackBarBehavior.floating))),
                                ],
                                if (ehASinc) ...[
                                  PCPEButton(label: 'Visualizar', icon: Icons.visibility_outlined, small: true, height: 30, outlined: true, onPressed: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Visualização em desenvolvimento.'), backgroundColor: PCPEColors.darkGray, behavior: SnackBarBehavior.floating))),
                                  const SizedBox(width: 6),
                                  PCPEButton(label: 'Sincronizar', icon: Icons.sync, small: true, height: 30, onPressed: () => _syncOcorrencia(realIndex)),
                                ],
                                if (ehConcluida) ...[
                                  PCPEButton(label: 'Visualizar', icon: Icons.visibility_outlined, small: true, height: 30, outlined: true, onPressed: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Visualização em desenvolvimento.'), backgroundColor: PCPEColors.darkGray, behavior: SnackBarBehavior.floating))),
                                  const SizedBox(width: 6),
                                  PCPEButton(label: 'Gerar PDF', icon: Icons.picture_as_pdf_outlined, small: true, height: 30, outlined: true, onPressed: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Gerar PDF (simulado).'), backgroundColor: PCPEColors.darkGray, behavior: SnackBarBehavior.floating))),
                                  const SizedBox(width: 6),
                                  PCPEButton(label: 'Enviar ao SPP', icon: Icons.send, small: true, height: 30, onPressed: () => _sendToSpp(realIndex)),
                                ],
                                if (ehEnviada) ...[
                                  PCPEButton(label: 'Visualizar', icon: Icons.visibility_outlined, small: true, height: 30, outlined: true, onPressed: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Visualização em desenvolvimento.'), backgroundColor: PCPEColors.darkGray, behavior: SnackBarBehavior.floating))),
                                  const SizedBox(width: 6),
                                  PCPEButton(label: 'Gerar PDF', icon: Icons.picture_as_pdf_outlined, small: true, height: 30, outlined: true, onPressed: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Gerar PDF (simulado).'), backgroundColor: PCPEColors.darkGray, behavior: SnackBarBehavior.floating))),
                                  const SizedBox(width: 6),
                                  PCPEButton(label: 'Imprimir', icon: Icons.print_outlined, small: true, height: 30, outlined: true, onPressed: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Imprimir (simulado).'), backgroundColor: PCPEColors.darkGray, behavior: SnackBarBehavior.floating))),
                                ],
                              ]),
                            ]),
                          ),
