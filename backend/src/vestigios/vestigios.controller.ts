...
export class VestigiosController {
  ...
  // ── Custódia (histórico imutável, sem PATCH/DELETE) ──────
  @Post(':vid/custodia') @HttpCode(HttpStatus.CREATED)
  addCustodia(@Param('ocorrenciaId') oid: string, @Param('vid') vid: string, @Body() dto: CreateMovimentacaoCustodiaDto, @CurrentUser() u: any) { return this.service.addMovimentacao(oid, vid, dto, u); }

  @Get(':vid/custodia')
  listHistorico(@Param('ocorrenciaId') oid: string, @Param('vid') vid: string, @CurrentUser() u: any) { return this.service.listHistorico(oid, vid, u); }
}