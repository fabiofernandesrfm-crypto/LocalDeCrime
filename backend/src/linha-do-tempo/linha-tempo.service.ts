...
export class LinhaTempoService {
  ...
  async getTimeline(ocorrenciaId: string, currentUser: any, query?: { categoria?: string; ordem?: string }): Promise<TimelineEventDto[]> {
    ...
    const ocorrencia = await this.prisma.ocorrencia.findUnique({
      ...
      include: { usuario: { select: { id: true, nome: true, matricula: true, cargo: true } } },
    });
    ...
    for (const p of pessoas) {
      eventos.push({
        id: `pessoa-${p.id}`,
        tipo: 'PESSOA_ADICIONADA',
        categoria: 'PESSOA',
        titulo: p.nome || 'Pessoa não identificada',
        descricao: `${p.tipoEnvolvimento}${p.identificada ? '' : ' (não identificada)'}`,
        dataHora: p.criadoEm.toISOString(),
        entidadeId: p.id,
        entidadeTipo: 'PESSOA',
        usuario: null,
    ...
    for (const v of veiculos) {
      eventos.push({
        ...
        titulo: v.placa || `${v.marca || ''} ${v.modelo || ''}`.trim() || 'Veículo',
        descricao: v.situacao || null, dataHora: v.criadoEm.toISOString(),
        entidadeId: v.id, entidadeTipo: 'VEICULO',
    ...
    for (const o of objetos) {
      eventos.push({
        ...
        titulo: o.descricao || o.categoria || 'Objeto',
        descricao: o.marca && o.modelo ? `${o.marca} ${o.modelo} (x${o.quantidade})` : `Quantidade: ${o.quantidade}`,
    ...
    // ── Vestígios (adicionado, acondicionado, lacrado) ──────
    const vestigios = await this.prisma.vestigioOcorrencia.findMany({
      where: { ocorrenciaId },
      select: {
        id: true, categoria: true, descricao: true, coletado: true, situacao: true,
        criadoEm: true, acondicionado: true, acondicionadoEm: true,
        numeroLacre: true, lacradoEm: true,
        criadoPor: { select: { id: true, nome: true, matricula: true, cargo: true } },
      },
      orderBy: { criadoEm: 'asc' },
    });
    for (const v of vestigios) {
      eventos.push({
        ...
        titulo: v.categoria || v.descricao || 'Vestígio',
        descricao: v.coletado ? 'Coletado' : 'Não coletado',
      ...
      if (v.acondicionado && v.acondicionadoEm) {
        eventos.push({
          id: `acond-${v.id}`, tipo: 'VESTIGIO_ACONDICIONADO', categoria: 'VESTIGIO',
          titulo: 'Vestígio acondicionado', descricao: v.descricao || v.categoria || null,
          dataHora: v.acondicionadoEm.toISOString(), entidadeId: v.id, entidadeTipo: 'VESTIGIO', usuario: null, metadata: null,
        });
      }
      if (v.numeroLacre && v.lacradoEm) {
        eventos.push({
          id: `lacre-${v.id}`, tipo: 'VESTIGIO_LACRADO', categoria: 'VESTIGIO',
          titulo: `Lacre ${v.numeroLacre}`, descricao: v.descricao || v.categoria || null,
          dataHora: v.lacradoEm.toISOString(), entidadeId: v.id, entidadeTipo: 'VESTIGIO', usuario: null,
          metadata: { numeroLacre: v.numeroLacre },
        });
      }
    }
    ...
    const custodiaList = await this.prisma.movimentacaoCustodiaVestigio.findMany({
      ...
      include: {
        registradoPor: { select: { id: true, nome: true, matricula: true, cargo: true } },
        vestigio: { select: { id: true, descricao: true, categoria: true } },
      },
    ...
    for (const c of custodiaList) {
      eventos.push({
        ...
        titulo: c.tipoMovimentacao,
        descricao: [c.origem, c.destino].filter(Boolean).join(' → ') || null,
    ...
    for (const a of anexos) {
      eventos.push({
        ...
        titulo: a.descricao || a.categoria || 'Anexo',
        descricao: a.arquivoOriginalNome,
  ...
  private _mapOcorrencia(o: any): TimelineEventDto {
    return {
      ...
      dataHora: o.criadoEm.toISOString(),
      entidadeId: o.id, entidadeTipo: 'OCORRENCIA',
